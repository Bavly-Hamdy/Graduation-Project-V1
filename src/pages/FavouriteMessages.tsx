...
import { Star, ArrowLeft, Copy, Trash2, User, Bot } from 'lucide-react';
...
const FavouriteMessages = () => {
  ...
  useEffect(() => {
    ...
    onValue(favRef, (snapshot) => {
      ...
      if (favData) {
        const messagesList = Object.entries(favData).map(([firebaseId, data]: [string, any]) => ({
          id: data.id,
          firebaseId,
        ...
        setFavouriteMessages(messagesList);
      } else {
  ...
  const handleDelete = async (firebaseId: string) => {
    try {
      await remove(ref(realTimeDb, `users/${userId}/favouriteMessages/${firebaseId}`));
      toast({
        title: language === 'en' ? "Removed" : "تم الحذف",
        description: language === 'en' ? "Message removed from favorites" : "تم حذف الرسالة من المفضلة"
      });
    } catch (error) {
      toast({
        title: language === 'en' ? "Error" : "خطأ",
        ...
        >
          ...
          <div>
            <h1 className="text-fluid-2xl font-bold text-gradient">
              {language === 'en' ? 'Favourite Messages' : 'الرسائل المفضلة'}
            </h1>
            <p className="text-muted-foreground">
              {language === 'en' 
                ? 'Your favourite messages from conversations'
                : 'رسائلك المفضلة من المحادثات'}
            </p>
          </div>
        </motion.div>
          ...
          {loading ? (
            <div className="text-center py-8">
              ...
              <p className="mt-4 text-muted-foreground">
                {language === 'en' ? 'Loading favourite messages...' : 'جاري تحميل الرسائل المفضلة...'}
              </p>
            </div>
            ...
            >
              ...
              <h3 className="text-lg font-medium mb-2">
                {language === 'en' ? 'No favourite messages' : 'لا توجد رسائل مفضلة'}
              </h3>
              <p className="text-muted-foreground mb-6">
                {language === 'en' 
                  ? 'Mark important messages as favorites to see them here'
                  : 'ضع علامة على الرسائل المهمة كمفضلة لتراها هنا'}
              </p>
            ...
            favouriteMessages.map((message, index) => (
              <motion.div
